...
3. **Domain** — the site uses `https://www.fourbrothersexteriors.ca` for canonical URLs, Open Graph tags, sitemap, robots.txt, and JSON-LD, centralized in `SITE_URL` in `src/lib/config.ts`. Update that one constant if the domain ever changes.
